...
% Assume svp function is defined elsewhere
...
figure;
hold on;
colors = ['b', 'g', 'r', 'c', 'm']; % Color for each line
for idx = 1:length(radii_initial)
    plot(t, radius_matrix(:, idx), 'Color', colors(idx), 'DisplayName', [num2str(radii_initial(idx)*10^6), '\mum']);
end
hold off;
legend('show');
xlabel('Time (s)');
