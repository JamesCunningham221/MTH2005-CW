...
for v = 1:length(w_values)
    ...
    for n = 1:n_steps
        [k1_R, k1_S] = five(r(n), s(n), w);
        [k2_R, k2_S] = five(r(n) + (dt/2)*k1_R, s(n) + (dt/2)*k1_S, w);
        [k3_R, k3_S] = five(r(n) + (dt/2)*k2_R, s(n) + (dt/2)*k2_S, w);
        [k4_R, k4_S] = five(r(n) + dt*k3_R, s(n) + dt*k3_S, w);
