...
initial_R_values = linspace(1e-6, 1e-7, 100);       % Initial droplet radius values (m)
initial_N_values = linspace(100e6,100e7 , 100);     % Initial droplet number concentration values (cm^-3)
initial_w_values = linspace(0.1, 1, 100);           % Define the values of initial vertical velocity
dt = 0.01;                                          % Time step (s)
...
figure;
subplot(2, 1, 1);
loglog(initial_N_values, max_R_N, '-|');
...
subplot(2, 1, 2);
loglog(initial_N_values, max_S_N, '-|');
xlabel('Initial Droplet Number Concentration (cm^{-3})');
ylabel('Max Supersaturation');
